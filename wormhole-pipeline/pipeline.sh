...
git_push() {
    GIT_PUSH_TIMEOUT=${GIT_PUSH_TIMEOUT:-30}
    GIT_PUSH_RETRIES=${GIT_PUSH_RETRIES:-2}
    GIT_TERMINAL_PROMPT=0
    local git_out git_rc
    git_out=""
    git_rc=0
    for attempt in $(seq 1 $GIT_PUSH_RETRIES); do
        git_out=$(timeout "$GIT_PUSH_TIMEOUT" \
            git -c core.hooksPath=/dev/null \
                -c 'url.git@github.com:.insteadOf=https://github.com/' \
                -C "$1" push origin "$2" 2>&1) || git_rc=$?
        if [[ $git_rc -eq 0 ]]; then
            break
        fi
        log "git push attempt ${attempt} failed (rc=${git_rc}), retrying..."
        sleep $((attempt * 2))
    done
    echo "$git_out"
    return $git_rc
